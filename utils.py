...
def hex_to_bytes(hex_str: str) -> bytes:
    return bytes.fromhex(hex_str or '')
...
def hkdf(length: int, derive_from: bytes, salt: Optional[bytes] = None, context: Optional[bytes] = None) -> bytes:
    if length < 1:
...
def ed25519_validate(public_key: bytes, signature: bytes, message: bytes) -> bool:
    try:
