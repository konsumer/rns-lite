...
def get_identity_destination_hash(identityPublic:bytes, full_name:str = 'lxmf.delivery') -> bytes:
  """
...
def validate_announce(packet:dict) -> Union[bool, dict]:
  ...
  if not len(packet['data']) > 64 + 10 + 10 + 64:
    app_data = None
  if not ed25519_validate(signature, signed_data, public_key[32:64], ):
    return False
  return {
    'app_data': app_data,
    'name_hash': name_hash,
...
def message_decrypt(packet:dict, receiver_pub:bytes, ratchets=[]) -> dict:
  ...
  for ratchet in ratchets:
    if len(ratchet) != 32:
      continue
    try:
      derived_key = hkdf(x25519_exchange(ratchet, peer_pub_bytes), 64, identity_hash)
...
def build_announce(identity_priv:bytes, identity_pub:bytes = None, destination_hash:bytes = None, ratchet_priv:bytes = None, ratchet_pub:bytes = None, full_name:str = 'lxmf.delivery', app_data:bytes = b'') -> bytes:
  """
  ...
  destination_hash = destination_hash or get_identity_destination_hash(identity_pub, full_name)
...
def build_data(plaintext:bytes, receiver_identity_pub:bytes, receiver_ratchet_pub:bytes, full_name:str = 'lxmf.delivery') -> bytes:
  """
...
def lxmf_parse(decrypted_data:bytes, packet_destination_hash:bytes, sender_pub:bytes) -> Union[bool, dict]:
  """
...
def lxmf_build(content:bytes, source_priv:bytes, destination_hash:bytes, source_hash:bytes = None, timestamp:float = None, fields:dict = None) -> bytes:
  """
